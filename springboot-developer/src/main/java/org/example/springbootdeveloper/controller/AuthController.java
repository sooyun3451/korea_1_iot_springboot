...
// 1. AuthController: 인증(Authentication) & 인가(Authorization)와 관련된 모든 로직을 처리하는 컨트롤러
// - 로그인, 로그아웃, 토큰 발급 등 사용자 인증과 관련된 기능을 포함

// 2. UserController: 사용자 관리와 관련된 작업을 처리
...
public class AuthController {
    ...
    public ResponseEntity<ResponseDto<UserSignUpResponseDto>> signUp(@Valid @RequestBody UserSignUpRequestDto userRequestDto) {
        try {
            ResponseDto<UserSignUpResponseDto> result = authService.signup(userRequestDto);
    ...
    public ResponseEntity<ResponseDto<UserSignInResponseDto>> signIn(@Valid @RequestBody UserSignInRequestDto userLoginRequestDto) {
        try {
            ResponseDto<UserSignInResponseDto> result = authService.signIn(userLoginRequestDto);
            return ResponseEntity.status(HttpStatus.OK).body(result);
        } catch(Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(null);
        }
