...
public class m_api {
    ...
    // : 유지보수성 - 코드 변경이 API 에 의해 중앙에서 관리, 다른 기능의 영향을 최소화

    // 2) API 구성요소
    // - 엔드포인트(Endpoint): API 가 연결되는 특정 URL 주소
    // - 메서드(Method): API 가 수행하는 작업, HTTP 메서드를 주로 사용(GET, POST, PUT, DELETE)
    // - Request: 클라이언트가 API 로 보내는 요청
    ...
    // - 접근 제어: 특정 웹 사이트나 데어터에 대한 접근을 차단하거나 제한

    // === 3. REST API ===
    // : REST(Representational State Transfer)
    // : HTTP 프로토콜을 기반으로 클라이언트와 서버간의 상호작용을 규정하는 아키텍처 스타일
    // : "자원"의 "표현"으로 "상태"를 전달하는 아키텍처

    // cf) 아키텍처 스타일: 네트워크 자원을 정의하고 처리하는 방법 전반
    ...
    // 5) 일관된 인터페이스: API 가 일관된 방식으로 설계되어야 함
    // 6) 코드 온 디멘드: (선택적)
    ...
    // - URL 은 리소스(자원)의 주소를 나타냄

    // 자원: URI 로 자원을 표현
    // 행위: 자원에 대한 행위는 HTTP Method 로 표현
    // 표현: 응답 자원의 형태를 JSON, XML 등의 형태로 나타냄

    // === 4. RESTful API ===
    // : URI 로 자원을 표현하는데 집중!하고 자원의 상태(행위)에 대한 정의는 HTTP Method 로 하는 것이 REST 하게 API 를 설계하는 규칙

    // - URI 로 자원(리소스)를 표현
    // - 자원에 대한 행위는 HTTP Method(GET, POST, PUT, DELETE)로 표현

    // REST API 사용 방법
    // 1. URL 에는 동사를 쓰지 않고, 자원을 표시
    ...
    // 2) PATCH: 리소스의 일부만 수정하는데 사용, 전체 리소스를 보내는 대신 수정하는 부분만 전송
    // >> 기존 리소스 데이터는 유지, 제공된 부분만 업데이터 됨
    // >> 멱등성을 가지지 않음
    //      : 여러 번 보내면 동일한 결과를 보장하지 X

    // cf) RESTful API 의 추가적인 특징
    // - /(슬래시)는 계층 관계를 나타님
    ...
    // 1) 헤더(Header): 요청이나 응답에 대한 추가 정보
    // EX) 메타데이터: 데이터의 타입, 보안 등을 명시
    //      Content-Type: text/html(Servlet)
    //      Content-Type: application/json
    // 2) 바디(Body, 본문): 요청이나 응답의 실제 데이터가 포함
}
