...
public class v_project_structure {
    ...
    /*
        1. common
            ...
            EX) DateUtils, Constants, CustomException(과) 같은 클래스들이 포함

        2. config
            Spring Boot 프로젝트의 설정 관련 클래스를 포함
            & 각종 설정을 정의하여 애플리케이션의 동작 방식을 제어
            EX) WebSecurityConfig(는) Spring Security 설정을 담당하여 애플리케이션의 인증 및 권한 부여 방식을 설정

        3. controller
            - 클라이언트의 요청을 받아 처리하는 컨트롤러 클래스들을 포함
            - Spring(의) @RestController 혹은 @Controller 애노테이션이 작성된 클래스들이 위치
                , 클라이언트가 보낸 요청을 처리하고 적절한 응답을 반환
            EX) UserController(가) 있다면 /user(와) 관련된 요청을 처리
                 , 각 요청 메서드(GET, POST, PUT, DELETE)에 맞춰 메소드를 제공(매핑 - 디스패처 서블릿이)

        4. dto (Data Transfer Object)
            - 데이터 전송 객체를 포함, 클라이언트와 서버 간의 데이터 교환을 위한 객체를 정의
            - Request(와) Response 폴더로 나뉨
        ...
        8. repository
            ...
            - 보통 JpaRepository(를) 확장하여 사용하며, 데이터베이스와 직접 통신
            EX) UserRepository: User 엔티티와 연관된 데이터를 CRUD 하는 메서드를 정의

        9. service
            - 비즈니스 로직을 담당하는 서비스 클래스들이 위치
            - 보통 @Service 애노테이션이 작성
                , 컨트롤러에서 전달받은 요청을 처리하고, 리포지토리와 연동하여 데이터를 관리
