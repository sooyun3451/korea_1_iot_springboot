...
public class p_스프링시큐리티 {
    ...
    // 1) SecurityFilterChain: 스프링 시큐리티의 요청에 대한 보안 처리 과정에서 여러 필터를 순차적으로 적용시킴
    ...
    // === 스프링 시큐리티의 절차(인증 처리) ===
    // 1. HTTP 요청: 사용자가 폼에 아이디 & 패스워드 입력 + 로그인 버튼 클릭
    // 2. HTTPServletRequest(에) 아이디와 비밀번호 정보가 전달: "AuthenticationFilter"(가) 전달된 아이디와 비밀번호의 유효성 검사를 실시
    // 3. 유효성 검사 완료 시 실체 구현체인 UsernamePasswordAuthenticationToken(을) 만들어 전달: 전달받은 인증용 객체를 "AuthenticationManager"(에게) 전달
    // 4. UsernamePasswordAuthenticationToken(을) "AuthenticationProvider"(에) 전달
    // 5. 사용자 아이디를 UserDetailService(에) 전달: UserDetailService(는) 사용자 아이디로 찾은 사용자 정보를 UserDetails 객체로 만들어 AuthenticationProvider(에) 전달
    // 6. DB에 있는 사용자 정보를 가져옴
    // 7. 입력 정보와 UserDetails 정보를 비교해 실제 인증 처리를 진행
    // 8. 인증이 완료되면 SecurityContextHolder(에) Authentication(을) 저장: 인증 성공 유무에 따라 핸들러 실행
}
