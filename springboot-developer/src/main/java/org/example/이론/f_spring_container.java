...
public class f_spring_container {
    ...
    // : 애플리케이션 내에서 객체(빈)의 생명주기와 설정을 관리
    ...
    // : 개발자에 의해 명시적으로 정의 | 스프링에 의해 자동으로 감지되어 정의
    ...
    // +) 빈 선언 방법
    // XML 파일 사용, 어노테이션 사용(@Component, @Service, @Repository 등), 자바 설정 클래스에서 @Bean 어노테이션 사용
}
...
// == 이식 가능한 셔비스 추상화 (PSA: Portable Service Abstraction) ==
