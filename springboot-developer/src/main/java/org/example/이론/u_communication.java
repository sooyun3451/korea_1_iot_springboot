...
public class u_communication {
    /*
        application server(애플리케이션 서버) - 8080
            ...
            - 작동방식
                : 클라이언트가 요청한 데이터나 로직을 처리하고 직접적인 통신보다는 주로 API(를) 통해 JSON 형태의 데이터를 반환(REST API - 데이터 반환)
        web server - 3000
            : React(React 내부의 웹 서버를 사용)
            : 정적 파일(HTML, CSS, JS) 등을 제공, 주로 웹 페이지를 렌더링하는 용도
            - 작동방식
                : 클라이언트로부터 요청을 받고 애플리케이션 서버의 데이터를 받아와서 사용자에게 보여주는 역할을 담당
        ...
        - @GetMapping("/api/v1/menus")로 매핑된 메서드가 있다면 해당 메서드가 요청을 받아 필요한 데이터를 처리하고 응답을 생성하여 반환

        == 6. 응답 전송 ==
        - 백엔드 서버는 HTTP 응답을 생성하여 요청을 보낸 프론트엔드 서버로 데이터를 전송
            : JSON 데이터 형식으로 응답
        - 응답 해더에는 CORS 관련 헤더(프론트엔드 경로로 응답 할 수 있음)가 포함
            : 프론트엔드가 응답을 정상적으로 수신할 수 있도록 설정

        == 7. 프론트엔드의 응답 처리 ==
        - 프론트엔드의 axios(는) 백엔드 서버의 응답을 받아 데이터를 가공하고 화면에 표시하거나 다른 로직에 사용
        - 예외적으로 요청이 실패하거나 CORS 오류가 발생하면 axios(의) catch 블록에서 에러 처리
    */
}
