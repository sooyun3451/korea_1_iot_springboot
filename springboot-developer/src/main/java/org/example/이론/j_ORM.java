...
public class j_ORM {
    ...
    // : 객체와 관계형 데이터베이스 간에 데이터를 매핑해주는 기술
    ...
    // >> 반복적인 SQL 작성 없이 데이터 조작 가능, DBMS 에 독립적
    ...
    // 3. ORM 에서 사용되는 주요 어노테이션
    // @Entity: 클래스를 DB 테이블과 매핑할 때 사용
    // - 옵션 없이 사용 가능
    // +) name 옵션: 엔티티 이름을 지정, 테이블명과 다를 경우 명시
    //  @Entity(name = "userTable")
    //  public class user {}

    // @ID: 해당 필드를 DB 테이블의 기본 키로 지정
    // - 옵션 없이 사용, 다른 어노테이션과 함께 기본 키 생성 방식이나 타입 설정 가능
    //  +) 주로 GeneratedValue 와 함께 사용

    // @GeneratedValue: 기본 키 자돋 생성 전략 설정(기본카 자동으로 만들어주는 것)
    // +) 다양한 옵션이 있음
    //  - strategy 옵션: GenerationType.IDENTITY(auto_increment)

    // @Column: 필드를 특정 테이블 열과 매핑, 생략 시 기본으로 필드 이름이 열 이름으로 사용
    // +) 다양한 옵션이 있읍
    //  - name 옵션: 열 이름 지정
    ...
    // +) 생략 시 기본으로 클래스 이름이 테이블 이름으로 명시
    // +) name 옵션: 이름이 다를 시 명시

    // cf) 옵션은 '옵션명 = 옵션사항'으로 명시, 각 옵션은 ,(콤마)로 구분
    // @Column(name = "user_password", nullable = false, unique = true)

    // cf) JPA(ORM, 객체와 RDMS 연결) VS MyBatis(SQL Mapper, SQL 중심 접근)
    ...
    // 애플리케이션 설정: 연결할 DB, 사용자 정보, 비밀번호 등을 설정
}
